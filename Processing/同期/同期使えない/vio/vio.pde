...
void serialEvent(Serial p) {
  ...
  if (inString.equals("stop")) {
    currentPlaybackID++; // ★再生IDを更新し、過去に予約された音符をすべて無効化
...
// ============================
// 🎻 ヴァイオリン用インストゥルメント (修正あり)
// ============================
class ViolinInstrument implements Instrument {
  Oscil wave;
  ADSR adsr;
  int myPlaybackID; // ★追加：この音が予約された時の再生ID
  ...
  ViolinInstrument(float frequency, float maxAmp, Waveform wf, int id) {
    ...
    wave.patch(adsr);
    myPlaybackID = id; // 予約時のIDを記録
  }

  void noteOn(float duration) {
    // ★現在の再生IDと一致している場合のみ音を出す（古い予約は無視される）
    if (myPlaybackID == currentPlaybackID) {
      adsr.noteOn();
      adsr.patch(out);
      activeInsts.add(this); // 強制停止用にリストへ追加
    }
  }

  void noteOff() {
    // ★現在の再生IDと一致している場合のみ通常の終了処理を行う
    if (myPlaybackID == currentPlaybackID) {
      adsr.noteOff();
